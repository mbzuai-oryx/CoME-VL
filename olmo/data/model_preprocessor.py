...
class MultiModalPreprocessor:
    ...
    image_token_length_w2: int = 7
    image_token_length_h2: int = 7
    image_patch_size: int = 16
    image_patch_size2: int = 14
    ...
    #     image_patch_size2 = self.image_patch_size2

    #     if isinstance(base_image_input_size, int):
    #         base_image_input_size = (base_image_input_size, base_image_input_size)

    #     base_image_input_d = image_patch_size
    #     base_image_input_d2 = image_patch_size2
    #     tokens_per_image = image_token_length_w * image_token_length_h
    #     image_base_patch_w = base_image_input_size[1] // base_image_input_d
    #     image_base_patch_h = base_image_input_size[0] // base_image_input_d

    #     image_base_patch_w2 = base_image_input_size[1] // base_image_input_d2
    #     image_base_patch_h2 = base_image_input_size[0] // base_image_input_d2
    ...
    #         patches2 = pixels_to_patches(resized, image_patch_size2)
    #         img_mask2 = pixels_to_patches(img_mask, image_patch_size2)
    ...
    #             per_row2 = np.concatenate([per_row2, [self.image_col_token_id]], 0, dtype=np.int32)
    #         extra_tokens = np.tile(per_row, [image_token_length_h])
    #         extra_tokens2 = np.tile(per_row2, [image_token_length_h])
    #         joint = [
    ...
    #         joint2 = np.concatenate(joint2, 0, dtype=np.int32)
    #         return np.expand_dims(patches, 0), joint, None, img_mask, joint2, None
    ...
    #         total_margin_pixels2 = base_image_input_d2*(right_margin + left_margin)  # pixels removed per dim
    #         crop_patches2 = base_image_input_size[0] // base_image_input_d2  # patches per crop dim
    #         crop_window_patches2 = crop_patches2 - (right_margin + left_margin)  # usable patches
    #         crop_window_size2 = crop_window_patches2 * base_image_input_d2
    ...
    #         # === build second-scale patch_ordering2 (mirrors first-scale) ===
    #         patch_ordering_arr2 = []

    #         on2 = 0
    #         for i in range(tiling2[0]):
    #             y0 = i * crop_window_size2
    #             crop_y0_2 = 0 if i == 0 else (left_margin // self.image_pooling_h)

    #             crop_h2 = image_base_patch_h2 - (right_margin + left_margin)
    #             if i == 0:
    #                 crop_h2 += left_margin
    #             if i == (tiling2[0] - 1):
    #                 crop_h2 += right_margin

    #             for j in range(tiling2[1]):
    #                 x0 = j * crop_window_size2
    #                 crop_x0_2 = 0 if j == 0 else (left_margin // self.image_pooling_w)

    #                 crop_w2 = image_base_patch_w2 - (right_margin + left_margin)
    #                 if j == 0:
    #                     crop_w2 += left_margin
    #                 if j == (tiling2[1] - 1):
    #                     crop_w2 += right_margin

    #                 pooled_w2 = (crop_w2 + self.image_pooling_w - 1) // self.image_pooling_w
    #                 pooled_h2 = (crop_h2 + self.image_pooling_h - 1) // self.image_pooling_h

    #                 after_padding_width2  = image_token_length_w - pooled_w2 - crop_x0_2
    #                 after_padding_height2 = image_token_length_h - pooled_h2 - crop_y0_2

    #                 patch_ordering_arr2.append(
    #                     np.pad(
    #                         np.reshape(
    #                             np.arange(on2, on2 + pooled_h2 * pooled_w2, dtype=np.int32),
    #                             (pooled_h2, pooled_w2)
    #                         ),
    #                         [[crop_y0_2, after_padding_height2],
    #                         [crop_x0_2, after_padding_width2]],
    #                         constant_values=-1, mode='constant'
    #                     )
    #                 )

    #                 on2 += pooled_h2 * pooled_w2

    #         # flatten and reorder to left-to-right (tile transpose) like scale-1
    #         patch_ordering2 = np.stack(patch_ordering_arr2)
    #         patch_ordering2 = np.reshape(patch_ordering2, [-1])
    #         valid2 = patch_ordering2 >= 0

    #         patch_ordering2_rh = np.reshape(
    #             patch_ordering2,
    #             [tiling2[0], tiling2[1], image_token_length_h, image_token_length_w]
    #         )
    #         patch_ordering2_rh = np.transpose(patch_ordering2_rh, [0, 2, 1, 3])
    #         patch_ordering2_rh = np.reshape(patch_ordering2_rh, [-1])

    #         patch_ordering2[valid2] = patch_ordering2_rh[patch_ordering2_rh >= 0]
    #         # === end second-scale patch_ordering2 ===
    ...
    #         h2 = get_num_patches(tiling2[0], self.image_pooling_h)
    #         w2 = get_num_patches(tiling2[1], self.image_pooling_w)
    #         per_row = np.full(
    ...
    #         )
    #         per_row2 = np.full(
    #             (w2 // self.image_pooling_w,),
    #             self.image_patch_token_id,
    #             dtype=np.int32
    #         )
    #         if self.use_col_tokens:
    #             per_row = np.concatenate([per_row, [self.image_col_token_id]], 0)
    #             per_row2 = np.concatenate([per_row2, [self.image_col_token_id]], 0)
    ...
    #         joint2 = np.tile(per_row2, [h // self.image_pooling_h])
    #         joint2 = [
    #             [self.image_start_token_id],
    #             joint2,
    #             [self.image_end_token_id]
    ...
    #         resized2, _ = self.resize_image(image, base_image_input_size, is_training, rng)
    #         resized2 = self._normalize(resized2)
    #         resized2 = pixels_to_patches(resized2, image_patch_size2)
    #         # breakpoint()
    #         # patches2 = np.concatenate([np.expand_dims(resized2, 0), patches2], 0)
    ...
    #             -1
    #         )
    #         patch_ordering2 = np.where(
    #             patch_ordering2 >= 0,
    #             patch_ordering2 + tokens_per_image,
    #             -1
    #         )
    #         patch_ordering = np.concatenate([np.arange(0, tokens_per_image), patch_ordering], 0)
    #         patch_ordering2 = np.concatenate([np.arange(0, tokens_per_image), patch_ordering2], 0)
    #         per_row = np.full(
    #             (image_token_length_w,),
    #             self.image_patch_token_id,
    #             dtype=np.int32
    ...
    #         joint2 = np.concatenate(joint2, 0)

    #         img_mask = np.pad(img_mask, [[0, 1], [0, 0]], constant_values=-1)
    #         return patches, joint, patch_ordering, img_mask, joint2, patch_ordering2
    #     else:
    ...
    def image_to_patches_and_tokens(
        ...
        image_token_length_w = self.image_token_length_w
        image_token_length_h = self.image_token_length_h
        image_patch_size  = self.image_patch_size         # scale-1 patch size (pixels)
        image_patch_size2 = self.image_patch_size2        # scale-2 patch size (pixels)

        # --- NEW: hard-code scale-2 base size ---
        base_image_input_size2 = (224, 224)

        if isinstance(base_image_input_size, int):
            base_image_input_size = (base_image_input_size, base_image_input_size)

        # no int form for scale-2 since it's hard-coded to a tuple

        base_image_input_d  = image_patch_size
        base_image_input_d2 = image_patch_size2

        # tokens per image (GLOBAL segment) for scale-1
        tokens_per_image  = image_token_length_w * image_token_length_h

        # If the model defines an explicit scale-2 token grid, use it; else derive it.
        def ceil_div(a, b):  # integer ceil division
            return (a + b - 1) // b

        if hasattr(self, "image_token_length_w2") and hasattr(self, "image_token_length_h2"):
            image_token_length_w2 = self.image_token_length_w2
            image_token_length_h2 = self.image_token_length_h2
        else:
            # --- NEW: derive scale-2 grid from base_image_input_size2 (224x224) ---
            base_patches_w2 = base_image_input_size2[1] // base_image_input_d2
            base_patches_h2 = base_image_input_size2[0] // base_image_input_d2
            image_token_length_w2 = ceil_div(base_patches_w2, self.image_pooling_w)
            image_token_length_h2 = ceil_div(base_patches_h2, self.image_pooling_h)

        tokens_per_image2 = image_token_length_w2 * image_token_length_h2

        # # base patch counts at base resolution (for crop geometry)
        image_base_patch_w  = base_image_input_size[1]  // base_image_input_d
        image_base_patch_h  = base_image_input_size[0]  // base_image_input_d

        # --- NEW: use base_image_input_size2 for scale-2 base patch grid ---
        image_base_patch_w2 = base_image_input_size2[1] // base_image_input_d2
        image_base_patch_h2 = base_image_input_size2[0] // base_image_input_d2

        original_image_h, original_image_w = image.shape[:2]
        crop_size = base_image_input_size[0]
        # (scale-2 crops are not extracted here; only ordering is built)

        # -------------------------
        # Helper to build 1-D token rows: [START] + tiled per-row tokens + [END]
        def build_joint_1d(nw: int, nh: int) -> np.ndarray:
            per_row = np.full((nw,), self.image_patch_token_id, dtype=np.int32)
            if self.use_col_tokens:
                per_row = np.concatenate(
                    [per_row, np.array([self.image_col_token_id], dtype=np.int32)],
                    axis=0
                )
            extra_tokens = np.tile(per_row, nh).astype(np.int32).reshape(-1)
            return np.concatenate(
                [
                    np.array([self.image_start_token_id], dtype=np.int32),
                    extra_tokens,
                    np.array([self.image_end_token_id],   dtype=np.int32),
                ],
                axis=0,
            )
        # -------------------------
        ...
        if self.crop_mode == "resize":
            # Simple global-only path (scale-1)
            resized, img_mask = self.resize_image(image, base_image_input_size, is_training, rng)
            resized = self._normalize(resized)
            ...
            # --- NEW: also resize once for scale-2 (224x224) for parity/sanity ---
            resized2, _ = self.resize_image(image, base_image_input_size2, is_training, rng)
            resized2 = self._normalize(resized2)
            _ = pixels_to_patches(resized2, image_patch_size2)  # not returned; computed if needed downstream

            # Build global-only tokens for both scales
            joint  = build_joint_1d(image_token_length_w,  image_token_length_h)
            joint2 = build_joint_1d(image_token_length_w2, image_token_length_h2)

            return np.expand_dims(patches, 0), joint, None, img_mask, joint2, None

        if self.crop_mode == "overlap-and-resize-c2":
            # Discard this many patches (in pooled patch units) from (left/top, right/bottom)
            left_margin, right_margin = overlap_margins
            # Required for compatibility with image pooling (margins must align with pooling grid)
            assert left_margin  % self.image_pooling_w == 0 and right_margin % self.image_pooling_w == 0
            assert left_margin  % self.image_pooling_h == 0 and right_margin % self.image_pooling_h == 0

            # ---- Per-scale crop geometry ----
            total_margin_pixels  = base_image_input_d  * (right_margin + left_margin)
            crop_patches         = base_image_input_size[0]  // base_image_input_d
            crop_window_patches  = crop_patches - (right_margin + left_margin)
            crop_window_size     = crop_window_patches * base_image_input_d

            # --- NEW: scale-2 geometry from base_image_input_size2 (224x224) ---
            total_margin_pixels2 = base_image_input_d2 * (right_margin + left_margin)
            crop_patches2        = base_image_input_size2[0] // base_image_input_d2
            crop_window_patches2 = crop_patches2 - (right_margin + left_margin)
            crop_window_size2    = crop_window_patches2 * base_image_input_d2

            # ---- Tiling for both scales ----
            tiling = select_tiling(
                original_image_h - total_margin_pixels,
                original_image_w - total_margin_pixels,
                crop_window_size,
            ...
            tiling2 = select_tiling(
                original_image_h - total_margin_pixels2,
                original_image_w - total_margin_pixels2,
                crop_window_size2,
                max_crops
            )

            # ---- Resize once large enough for BOTH tilings ----
            src_h = max(tiling[0]*crop_window_size + total_margin_pixels,
                        tiling2[0]*crop_window_size2 + total_margin_pixels2)
            src_w = max(tiling[1]*crop_window_size + total_margin_pixels,
                        tiling2[1]*crop_window_size2 + total_margin_pixels2)

            src, img_mask = self.resize_image(image, [src_h, src_w], is_training, rng)
            src = self._normalize(src)

            # ==================== Scale-1: crops -> patches, mask, ordering ====================
            patches_arr, mask_arr, patch_ordering_arr = [], [], []

            # Sanity: after pooling, grid equals configured token grid
            assert ceil_div(crop_patches, self.image_pooling_h) == image_token_length_h
            assert ceil_div(crop_patches, self.image_pooling_w) == image_token_length_w

            on = 0
            for i in range(tiling[0]):
                y0 = i * crop_window_size
                crop_y0 = 0 if i == 0 else (left_margin // self.image_pooling_h)
                ...
                for j in range(tiling[1]):
                    x0 = j * crop_window_size
                    crop_x0 = 0 if j == 0 else (left_margin // self.image_pooling_w)
                    ...
                    pooled_w = ceil_div(crop_w, self.image_pooling_w)
                    pooled_h = ceil_div(crop_h, self.image_pooling_h)
            ...
            # To [n_crops, n_patches, pixels_per_patch]
            patches  = batch_pixels_to_patches(patches,  image_patch_size)
            img_mask = batch_pixels_to_patches(img_mask, image_patch_size).astype(np.float32).mean(axis=-1)

            # Flatten + reorder (tile transpose) to left->right order
            patch_ordering = np.reshape(patch_ordering, [-1])
            valid = patch_ordering >= 0
            patch_ordering_rh = np.reshape(
                patch_ordering,
            ...
            patch_ordering_rh = np.transpose(patch_ordering_rh, [0, 2, 1, 3]).reshape(-1)
            patch_ordering[valid] = patch_ordering_rh[patch_ordering_rh >= 0]

            # ==================== Scale-2: ordering only (mirrors scale-1) ====================
            patch_ordering_arr2 = []
            on2 = 0
            for i in range(tiling2[0]):
                y0 = i * crop_window_size2
                ...
                # --- NEW: use image_base_patch_h2 (224-derived) ---
                crop_h2 = image_base_patch_h2 - (right_margin + left_margin)
                if i == 0:
                    crop_h2 += left_margin
                if i == (tiling2[0] - 1):
                ...
                for j in range(tiling2[1]):
                    ...
                    # --- NEW: use image_base_patch_w2 (224-derived) ---
                    crop_w2 = image_base_patch_w2 - (right_margin + left_margin)
                    if j == 0:
                        crop_w2 += left_margin
                    if j == (tiling2[1] - 1):
                    ...
                    pooled_w2 = ceil_div(crop_w2, self.image_pooling_w)
                    pooled_h2 = ceil_div(crop_h2, self.image_pooling_h)

                    # NOTE: use the *scale-2* token grid here
                    after_padding_width2  = image_token_length_w2 - pooled_w2 - crop_x0_2
                    after_padding_height2 = image_token_length_h2 - pooled_h2 - crop_y0_2
                    ...
                    patch_ordering_arr2.append(
                        np.pad(
                            ...
                            [[crop_y0_2, after_padding_height2], [crop_x0_2, after_padding_width2]],
                            constant_values=-1, mode='constant'
                        )
                    )
                    on2 += pooled_h2 * pooled_w2

            patch_ordering2 = np.stack(patch_ordering_arr2).reshape(-1)
            valid2 = patch_ordering2 >= 0
            # NOTE: reshape with *scale-2* token grid
            patch_ordering2_rh = np.reshape(
                patch_ordering2,
                [tiling2[0], tiling2[1], image_token_length_h2, image_token_length_w2]
            )
            patch_ordering2_rh = np.transpose(patch_ordering2_rh, [0, 2, 1, 3]).reshape(-1)
            patch_ordering2[valid2] = patch_ordering2_rh[patch_ordering2_rh >= 0]

            # ==================== Build token vectors (global then crops) ====================
            def get_num_patches(num_tiles: int, pooling_size: int) -> int:
                if num_tiles > 1:
                    left_crop_window_patches   = ceil_div(crop_window_patches + left_margin, pooling_size) * pooling_size
                    middle_crop_window_patches = ceil_div(crop_window_patches,             pooling_size) * pooling_size
                    right_crop_window_patches  = ceil_div(crop_window_patches + right_margin, pooling_size) * pooling_size
                    return left_crop_window_patches + (num_tiles - 2) * middle_crop_window_patches + right_crop_window_patches
                else:
                    return ceil_div(crop_patches, pooling_size) * pooling_size

            def get_num_patches2(num_tiles: int, pooling_size: int) -> int:
                if num_tiles > 1:
                    left_crop_window_patches   = ceil_div(crop_window_patches2 + left_margin, pooling_size) * pooling_size
                    middle_crop_window_patches = ceil_div(crop_window_patches2,             pooling_size) * pooling_size
                    right_crop_window_patches  = ceil_div(crop_window_patches2 + right_margin, pooling_size) * pooling_size
                    return left_crop_window_patches + (num_tiles - 2) * middle_crop_window_patches + right_crop_window_patches
                else:
                    return ceil_div(crop_patches2, pooling_size) * pooling_size

            # total pooled patch grid for crops at each scale
            h  = get_num_patches( tiling[0],  self.image_pooling_h)
            w  = get_num_patches( tiling[1],  self.image_pooling_w)
            h2 = get_num_patches2(tiling2[0], self.image_pooling_h)
            w2 = get_num_patches2(tiling2[1], self.image_pooling_w)

            # tokens-per-row AFTER pooling and row count AFTER pooling
            nw_crops  = w  // self.image_pooling_w
            nh_crops  = h  // self.image_pooling_h
            nw_crops2 = w2 // self.image_pooling_w
            nh_crops2 = h2 // self.image_pooling_h

            # Build "global" tokens using their own grids
            joint_global  = build_joint_1d(image_token_length_w,  image_token_length_h)
            joint_global2 = build_joint_1d(image_token_length_w2, image_token_length_h2)
            # Build "crops" tokens for each scale
            joint_crops   = build_joint_1d(nw_crops,  nh_crops)
            joint_crops2  = build_joint_1d(nw_crops2, nh_crops2)

            # Concatenate as: [START global END] + [START crops END]
            joint  = np.concatenate([joint_global,  joint_crops],  axis=0).astype(np.int32)
            joint2 = np.concatenate([joint_global2, joint_crops2], axis=0).astype(np.int32)

            # ---- add global-first offsets to orderings and prepend each global range ----
            patch_ordering  = np.where(patch_ordering  >= 0, patch_ordering  + tokens_per_image,  -1)
            patch_ordering2 = np.where(patch_ordering2 >= 0, patch_ordering2 + tokens_per_image2, -1)

            patch_ordering  = np.concatenate([np.arange(0, tokens_per_image,  dtype=np.int32), patch_ordering],  axis=0)
            patch_ordering2 = np.concatenate([np.arange(0, tokens_per_image2, dtype=np.int32), patch_ordering2], axis=0)

            # ---- finalize mask (pad one row for the scale-1 global image) ----
            img_mask = np.pad(img_mask, [[0, 1], [0, 0]], constant_values=-1)
    ...
    def build_image_input_idx(
        ...
        if type_encoder=='enc1':
            tokens_per_image = self.image_token_length_w * self.image_token_length_h
        else:
            tokens_per_image = self.image_token_length_w2 * self.image_token_length_h2
    ...
    def build_image_input_idx2(
        self,
        image_tokens: np.ndarray,
        patch_order: np.ndarray,
        tokens_per_image: int,   # <<< pass 256 for s1, 196 for s2
    ):
        """Map patch_id -> token_position. Returns [num_images, tokens_per_image] int32; -100 for skipped."""
        # 1) positions of patch tokens in the flat token stream
        image_input_idx = (image_tokens == self.image_patch_token_id)
        image_input_idx = np.nonzero(image_input_idx)[0].astype(np.int32)

        if patch_order is not None:
            patch_order = np.reshape(patch_order, [-1]).astype(np.int32)
            valid = (patch_order >= 0)
            n_valid = int(valid.sum())

            # sanity: number of kept patches must match number of patch tokens we found
            assert image_input_idx.shape[0] == n_valid, (
                f"Found {image_input_idx.shape[0]} patch tokens but {n_valid} valid patch indices."
            )

            # reverse map: sorted position -> index in kept list
            sorted_patch_ixs = np.zeros([n_valid], dtype=np.int32)
            # patch_order[valid] are "sorted positions" in [0..n_valid-1]
            sorted_patch_ixs[patch_order[valid]] = np.arange(n_valid, dtype=np.int32)

            # expand back to full patch space (-1 where skipped)
            sorted_patch_ixs_ex = np.full(patch_order.shape, -1, dtype=np.int32)
            sorted_patch_ixs_ex[valid] = sorted_patch_ixs

            is_valid = (sorted_patch_ixs_ex >= 0).astype(np.int32)
            mapped   = image_input_idx[sorted_patch_ixs_ex * is_valid]   # index into kept positions
            image_input_idx = mapped * is_valid - 100 * (1 - is_valid)   # -100 for skipped

        total = image_input_idx.shape[0]
        assert total % tokens_per_image == 0, (
            f"Cannot reshape {total} elements into [-1, {tokens_per_image}]. "
            "Pass the correct tokens_per_image for this scale."
        )
        image_input_idx = np.reshape(image_input_idx, [-1, tokens_per_image]).astype(np.int32)
        return image_input_idx
    ...
    def preprocess(self, image, is_training: bool, rng=None):
        ...
        """
        # print(ad)
        crops, image_tokens, patch_ordering, img_mask, image_tokens2, patch_ordering2 = self.image_to_patches_and_tokens(
            image, is_training, rng)
        
        patch_idx = self.build_image_input_idx( image_tokens, patch_ordering, type_encoder='enc1')
        # breakpoint()
        patch_idx2 = self.build_image_input_idx( image_tokens2, patch_ordering2, type_encoder='enc2')
        # patch_idx2 = self.build_image_input_idx2( image_tokens2, patch_ordering2, tokens_per_image=49)
        return crops, image_tokens, patch_idx, img_mask, patch_idx2
    ...
    def __call__(
        ...
        for ix in range(n):
            ...
            crops, image_tokens, patch_idx, img_mask, patch_idx2 = self.preprocess(images[ix], is_training, rng)
            ...
            all_image_idx2.append(patch_idx2 + token_ix)
        ...
        image_input_idx2 = np.concatenate(all_image_idx2, 0)
        ...
        image_input_idx2 = np.where(image_input_idx2 < 0, image_input_idx2, image_input_idx2 + 1)
        out = {
            "input_tokens": input_ids,
            "images": images,
            "image_input_idx": image_input_idx,
            "image_input_idx2": image_input_idx2,
