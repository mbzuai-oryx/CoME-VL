...
class PixMoCount(Dataset):
    ...
    def __init__(self, split, sample=None, counting=False, keep_in_memory=False):
        # def __init__(self, split, sample=None, counting=False, keep_in_memory=False):
        dd = load_from_disk(join(PIXMO_DATASETS, "count"), keep_in_memory=keep_in_memory)

        if split == "train":
            # concat all splits for training
            assert isinstance(dd, DatasetDict), "Expected a DatasetDict on disk"
            splits = [dd[s].flatten_indices() for s in ("train", "test", "validation")]
            ds = concatenate_datasets(splits)
        else:
            ds = dd[split].flatten_indices()

        # remap image base path
        OLD_BASE = "/Med_Repo_Gen/data_cache/torch_datasets/pixmo_images"
        NEW_BASE = "/Document/molmo_code/data/torch_datasets/pixmo_images"

        def _fix_paths(batch):
            imgs = batch["image"]
            return {"image": [
                p.replace(OLD_BASE, NEW_BASE, 1) if isinstance(p, str) and p.startswith(OLD_BASE) else p
                for p in imgs
            ]}

        ds = ds.map(_fix_paths, batched=True, num_proc=16)

        # ✨ filter out rows whose image path does NOT exist
        def _keep_existing(batch):
            imgs = batch["image"]
            return [isinstance(p, str) and path_exists(p) for p in imgs]

        ds = ds.filter(_keep_existing, batched=True, num_proc=16)

        # optional subsample
        if sample is not None and sample > 0 and len(ds) > sample:
            ds = ds.select(range(sample))

        self.dataset = ds
        self.counting = counting
        self._split = split   # use private field, not self.split
