...
from olmo.cross_rope import SigLIP_DINO_CrossRoPE4D, LayerPoolRoPE
...
class OLMoSequentialBlock(OLMoBlock):
    ...
    def __init__(self, layer_id: int, config: ModelConfig, cache: BufferCache):
        ...
        #     4096, 
        #     config.effective_n_kv_heads * head_dim, 
        #     config.effective_n_kv_heads * head_dim, )  # 4096 + 1024 + 1024
            

        # # if getattr(config, "decoder_name", None) == "qwen3_4b":
        # #     self.fused_dims = 6144  # 4096 + 1024 + 1024
        # if not self.fused_dims : 
        #     self.fused_dims = (
        #         config.d_model,
        #         config.effective_n_kv_heads * head_dim,
        #         config.effective_n_kv_heads * head_dim,
        ...
        # print(123, config.d_model,
        #     config.effective_n_kv_heads * head_dim,
        #     config.effective_n_kv_heads * head_dim, config.effective_n_kv_heads, config.n_kv_heads, head_dim)
        # print(addd)
        self.att_proj = nn.Linear(
...
class PatchEmbeddingMerger(nn.Module):
    def __init__(self, in_dim1=2304, in_dim2=1024, out_dim=1024, align_patches=False):
        """
        Trainable merger for two patch embeddings.
        Args:
            in_dim1: input dim of first embedding
            in_dim2: input dim of second embedding
            out_dim: common projection dimension
            align_patches: if True, resample patches so both have same token length
        """
        super().__init__()
    ...
    def forward(self, x1, x2):
        """
        x1: [B, H, N1, in_dim1] - siglip
        x2: [B, H, N2, in_dim2] - dino
        """
        B, H, N1, _ = x1.shape
        _, _, N2, _ = x2.shape

        # project to common space
        x1 = self.proj1(x1)   # [B, H, N1, out_dim]
        # x2 = self.proj2(x2)   # [B, H, N2, out_dim]

        # align patch counts if requested
        if self.align_patches and N1 != N2:
            # interpolate x1 to match N2
            x1_reshaped = x1.view(B*H, N1, -1).transpose(1, 2)  # [B*H, out_dim, N1]
            x1_resized = F.interpolate(
                x1_reshaped, size=N2, mode="linear", align_corners=False
            ).transpose(1, 2).view(B, H, N2, -1)                # [B, H, N2, out_dim]
            x1 = x1_resized

        # concatenate along feature dim
        merged = torch.cat([x1, x2], dim=-2)       # [B, H+N, out_dim]

        # project back to out_dim
        # merged = self.out_proj(merged)             # [B, H, N, out_dim]
        return merged




# def LLN(x: torch.Tensor, ln: nn.LayerNorm, linear: nn.Linear) -> torch.Tensor:
#     """Linear(LN(x)) over the last dim; x can be [B, D] or [B, T, D]."""
#     return linear(ln(x))

# class WeightedLLNSum(nn.Module):
#     """
#     v̄ = Σ_k w_k · Linear(LN(z_{idx_k})), with variable index ranges.
#     Build one instance per stream if you want separate LN/Linear params.

#     Args:
#         d_in:  last-dim of each per-layer feature
#         d_out: projection size (default=d_in)
#         one_based: if True, indices are 1-based (paper-style)
#     """
#     def __init__(self, d_in: int, d_out: int | None = None, one_based: bool = True):
#         super().__init__()
#         self.one_based = one_based
#         self.ln   = nn.LayerNorm(d_in)
#         self.fc   = nn.Linear(d_in, d_out or d_in)

#     def forward(
#         self,
#         features: List[torch.Tensor],        # ordered per-layer tensors: [..., D]
#         indices: Sequence[int],              # e.g., range(1,25) or [19,20,21,22,23,24]
#         weights: torch.Tensor,               # shape [N]; logits or fixed weights
#         softmax_weights: bool = True,        # True ⇒ treat `weights` as logits
#     ) -> torch.Tensor:
#         """
#         Returns: v̄ with shape matching features[k] except last dim = d_out.
#         """
#         assert len(indices) == weights.numel(), "weights must match number of indices"
#         # (optional) softmax-normalize weights
#         w = weights.softmax(dim=0) if softmax_weights else weights

#         # Explicit sum: w1*LLN(z1) + ... + wN*LLN(zN)
#         out = None
#         for k, idx in enumerate(indices):
#             pos = idx - 1 if self.one_based else idx
#             term = w[k] * LLN(features[pos], self.ln, self.fc)
...
# minimal helper used by your original code
def LLN(x: torch.Tensor, ln: nn.LayerNorm, fc: nn.Linear) -> torch.Tensor:
    return fc(ln(x))

# try to use PyTorch's orthogonal parametrization (keeps weight on a Stiefel manifold)
try:
    from torch.nn.utils.parametrizations import orthogonal as _orthogonal_param
    _HAS_ORTH = True
except Exception:
    _HAS_ORTH = False
...
class WeightedLLNSum(nn.Module):
    """
    v̄ = Σ_k w_k · OrthogonalLinear(LN(z_{idx_k})), with variable index ranges.
    Build one instance per stream if you want separate LN/Linear params.
    ...
    def __init__(self, d_in: int, d_out: int | None = None, one_based: bool = True):
        ...
        # --- replace Linear with an orthogonal Linear ---
        fc = nn.Linear(d_in, d_out or d_in, bias=False)  # bias=False to keep a pure (semi-)isometry
        if _HAS_ORTH:
            # hard constraint: weight stays (semi-)orthogonal during training
            self.fc = _orthogonal_param(fc)
        else:
            # fallback: keep a standard Linear but initialize it orthogonally
...
class TwoStageWeightedLLNSum(nn.Module):
    def __init__(self, d_in: int, d_out: int | None = None, one_based: bool = True):
        super().__init__()
        self.one_based = one_based
        d_out = d_out or d_in
        
        # Single LayerNorm (shared for simplicity)
        self.ln = nn.LayerNorm(d_in)
        
        # Stage 1: Per-layer orthogonal projection
        fc1 = nn.Linear(d_in, d_in, bias=False)
        if _HAS_ORTH:
            self.fc1 = _orthogonal_param(fc1)
        else:
            nn.init.orthogonal_(fc1.weight)
            self.fc1 = fc1
        
        # Stage 2: Final orthogonal projection
        fc2 = nn.Linear(d_in, d_out, bias=False)
        if _HAS_ORTH:
            self.fc2 = _orthogonal_param(fc2)
        else:
            nn.init.orthogonal_(fc2.weight)
            self.fc2 = fc2

    def forward(self, features, indices, weights, softmax_weights=True):
        w = weights.softmax(dim=0) if softmax_weights else weights

        # Stage 1: weighted sum of per-layer projections
        merged = None
        for k, idx in enumerate(indices):
            pos = idx - 1 if self.one_based else idx
            projected = self.fc1(self.ln(features[pos]))  # Orth1(LN(z_k))
            term = w[k] * projected
            merged = term if merged is None else (merged + term)
        
        # Stage 2: final projection
        return self.fc2(self.ln(merged))  # Orth2(LN(merged))



class GEGLU(nn.Module):
    """Gated GELU: (XW1) ⊙ GELU(XW2)."""
...
class GatedMLPHead(nn.Module):
    """
    LN -> GEGLU(d_in->d_hidden) -> Dropout -> Linear(d_hidden->d_out)
    + optional residual if d_out == d_in.
    Works for last-dim features of any leading shape.
    """
    def __init__(self, d_in: int, d_out: int | None = None,
                 expansion: int = 4, dropout: float = 0.1, residual: bool = True,
                 final_ln: bool = False):
        super().__init__()
        self.d_in = d_in
        self.d_out = d_out or d_in
        self.residual = residual and (self.d_out == d_in)

        d_hidden = expansion * d_in

        self.ln = nn.LayerNorm(d_in)
        self.geglu = GEGLU(d_in, d_hidden)
        self.drop = nn.Dropout(dropout)
        self.fc_out = nn.Linear(d_hidden, self.d_out)
        self.final_ln = nn.LayerNorm(self.d_out) if final_ln else nn.Identity()
...
class MolmoVisionBackbone(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        if config.image_pooling_2d in {ImagePooling2DType.attention, ImagePooling2DType.attention_meanq}:
            ...
            self.image_pooling_2d_2 = ViTMultiHeadDotProductAttention(config, is_vit_layer=False, encoder2=True)
            input_dim2 = config.vision_backbone2.image_emb_dim
            ...
            self.image_pooling_2d = ViTMultiHeadDotProductAttention(cfg, is_vit_layer=False, encoder2=True)
            input_dim = cfg.vision_backbone.image_emb_dim

            # cfg.vision_backbone2.image_emb_dim *= 2
            # cfg.vision_backbone2.image_head_dim *= 2
            self.image_pooling_2d_2 = ViTMultiHeadDotProductAttention(cfg, is_vit_layer=False, encoder2=True)
            input_dim2 = cfg.vision_backbone2.image_emb_dim
            ...
            self.image_pooling_2d_2 = None
            nlayers2 = 1 if config.vit_layers2 is None else len(config.vit_layers2)
            input_dim2 = nlayers2 * config.vision_backbone2.image_emb_dim
            if config.image_pooling_2d_2 == ImagePooling2DType.stack:
                input_dim2 *= 4
        ...
        self.merger = PatchEmbeddingMerger(in_dim1=2304, in_dim2=1024, out_dim=1024, align_patches=False)
        ...
        if config.image_projector == ImageProjectType.mlpx2:
            ...
            self.image_projector2 = nn.ModuleList(
                [ImageProjectorMLP(mlp_config, input_dim), Residual(ImageProjectorMLP(config, input_dim2))]
            )
        elif config.image_projector == ImageProjectType.mlp:
            self.image_projector = ImageProjectorMLP(mlp_config, input_dim)
            self.image_projector2 = ImageProjectorMLP(mlp_config, input_dim2)
            ...
            self.image_projector2 = nn.Linear(
                input_dim2,
                config.d_model,
                bias=False,
                device=config.init_device,
        ...
        if self.config.vision_backbone2 :
            v_cfg2 = self.config.vision_backbone2
        ...
        if v_cfg.image_model_type == VisionBackboneType.openai:
            ...
            self.image_vit = SiglipVisionTransformer(config)#hwy?
        elif v_cfg.image_model_type == VisionBackboneType.dino:
            self.image_vit = DinoVisionTransformer(config)


        if v_cfg2.image_model_type == VisionBackboneType.openai:
            self.image_vit2 = VisionTransformer(config)
        elif v_cfg2.image_model_type == VisionBackboneType.siglip:
            self.image_vit2 = SiglipVisionTransformer(config)
        elif v_cfg2.image_model_type == VisionBackboneType.dino:
            self.image_vit2 = DinoVisionTransformer(config)
        ...
        if self.image_vit2:
            ...
            self.num_prefix_tokens2 = self.image_vit2.num_prefix_tokens
        ...
        if config.image_padding_embed:
            ...
            image_dim2 = v_cfg2.image_emb_dim#*len(self.config.vit_layers2)
            if config.image_padding_embed in ["pad_embed", "regress"]:
                ...
                self.pad_embed2 = nn.Parameter(
                    torch.zeros((image_dim2,), device=config.init_device))
                ...
                self.pad_embed2 = nn.Parameter( torch.zeros((2, image_dim2), device=config.init_device) )
                    
            else:
                raise ValueError(config.image_padding_embed)

        ## For feature marging
        self.pool1 = TwoStageWeightedLLNSum(d_in=self.input_dim, one_based=True)
        self.pool2 = TwoStageWeightedLLNSum(d_in=self.input_dim2, one_based=True)

        self.idx1 = list(range(0, 28))                  # 0..22
        self.alpha_logits = nn.Parameter(torch.zeros(len(self.idx1)))  # learnable; init uniform

        self.idx2 = list(range(10, 24))                 # 0..9
        self.beta_logits = nn.Parameter(torch.zeros(len(self.idx2)))   # learnable; init uniform


        self.mlp_after_v2 = GatedMLPHead(
            d_in=self.input_dim2,           # input width = vbar2 dim
            d_out=self.input_dim2,          # keep same dim; set to something else if you want
            expansion=4,      # 4x hidden (common, robust)
            dropout=0.1,      # tune 0.0–0.3
            residual=True,    # enabled when d_out==d_in
            final_ln=False    # set True if you want extra stability
        )
        
        self.cross_rope = SigLIP_DINO_CrossRoPE4D(
            d_siglip=self.input_dim, d_dino=self.input_dim2,
            d_model=config.d_model, n_head=8,
        ...
        self.pool_sig = LayerPoolRoPE(d_model=self.input_dim, n_head=8)
        self.pool_din = LayerPoolRoPE(d_model=self.input_dim2, n_head=8)
    ...
    def reset_connector_parameters(self):
        ...
        if self.image_pooling_2d_2 is not None:
            self.image_pooling_2d_2.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        # self.image_vit2.reset_parameters()
    ...
    def set_activation_checkpointing(self, strategy: Optional[ActivationCheckpointingStrategy]):
        ...
        if strategy in (ActivationCheckpointingStrategy.whole_layer, ActivationCheckpointingStrategy.vit_only):
            ...
            self.image_vit2.set_grad_checkpointing()
    ...
    def encode_image(self, images: torch.Tensor) -> torch.Tensor:
        ...
        N1 = 196  # Dino V3 No of patches

        mask = ~torch.all(images.view(B * T, N, D) == -1, dim=(1, 2), keepdim=True)

        images = images.view(B * T, N, D)
        image_features = self.image_vit(images)
        hidden_state_image_features2 = self.image_vit2(images)['hidden_states']
        # last_hidden_state_image_features = self.image_vit(images)['last_hidden_state']
        # breakpoint()
        ...
        # # breakpoint()
        # if cfg.vit_layers2 is not None:
        #     features2 = []
        #     for layer in cfg.vit_layers2:
        #         features2.append(hidden_state_image_features2[layer])
        #     image_features2 = torch.cat(features2, dim=-1)
        # else:
        #     image_features2 = hidden_state_image_features2[-1]

        cls_embed1: torch.Tensor = None
        cls_embed2: torch.Tensor = None
        # if self.num_prefix_tokens1 > 0:
        cls_embed2 = hidden_state_image_features2[-1][:, 0]
        # breakpoint()
        if self.num_prefix_tokens > 0:
            cls_embed1 = image_features[-1][:, 0]
            # image_features = image_features[:, 1:]

        image_features2 = torch.stack([x[:, 5:, :] for x in hidden_state_image_features2[1:] ], dim=0) # [:, 1 + model.config.num_register_tokens(4):, :]
        # breakpoint()
        image_features= self.pool1(image_features, self.idx1, self.alpha_logits, softmax_weights=True)
        image_features2= self.pool2(image_features2, self.idx2, self.beta_logits, softmax_weights=True)
        image_features2 = self.mlp_after_v2(image_features2)
        
        # image_features = self.pool_sig(image_features, self.idx1, self.idx1[-1], hw=(24, 24))
        # image_features2 = self.pool_din(image_features2, self.idx2, self.idx2[-1], hw=(14, 14))
        ...
        image_features2 = image_features2 * mask
        image_features2 = image_features2.unsqueeze(0)
        image_features2 = image_features2.view(B, T, N1, -1)

        cls_embed1 = cls_embed1.view(B, T, -1) if cls_embed1 is not None else None
        cls_embed2 = cls_embed2.view(B, T, -1) if cls_embed2 is not None else None

        return image_features, image_features2
    ...
    def forward(self, images: torch.Tensor, image_masks: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        ...
        image_features, image_features2 = self.encode_image(images)
        # breakpoint()
        # print(123, image_features.shape, image_features2.shape)
        ...
        if cfg.image_padding_embed:
            ...
            if cfg.image_padding_embed == "pad_embed":
                ...
                # if get_rank() == 0:
                #     pdb.set_trace()
                # print(image_features.shape, pad_embed[0].shape, all_pad.shape)
                # print(ad)
            ...
            #     all_pad = (image_masks == 0).to(dtype=torch.float32)
            #     partial_pad = torch.logical_and(image_masks < 1, torch.logical_not(all_pad.bool())).to(dtype=torch.float32)

            #     # 🔹 Ensure mask length matches sequence length (e.g., 13 → 26)
            #     if all_pad.shape[1] != image_features.shape[1]:
            #         factor = image_features.shape[1] // all_pad.shape[1]
            #         all_pad = all_pad.repeat_interleave(factor, dim=1)
            #         partial_pad = partial_pad.repeat_interleave(factor, dim=1)
        ...
        image_features2 = self.image_feature_dropout(image_features2)
        # print(22, image_features.shape)
        image_features = image_features.reshape(
            (batch_size, num_image) + cfg.image_num_patch + (-1,),
        )
        image_features2 = image_features2.reshape( (batch_size, num_image) + cfg.image_num_patch2 + (-1,), )
        # print(33, image_features.shape)
        ...
        if cfg.image_num_patch2[0] % cfg.image_pooling_h != 0 or cfg.image_num_patch2[1] % cfg.image_pooling_w != 0:
            pad_h = cfg.image_num_patch2[0] % cfg.image_pooling_h
            pad_w = cfg.image_num_patch2[1] % cfg.image_pooling_w
            # Pad so we can still pool mxn patches
            image_features2 = F.pad(
                image_features2,
                (0, 0, 0, pad_w, 0, pad_h, 0, 0, 0, 0),
            )
        # print(44, image_features.shape)
        # print(44, image_features2.shape)
        ...
        image_features2 = einops.rearrange(
            image_features2,
            'b n (h dh) (w dw) c -> (b n h w) (dh dw) c',
            dh=cfg.image_pooling_h,
            dw=cfg.image_pooling_w,
        )
        # print(55, image_features.shape)
        # print(55, image_features2.shape)
        ...
        if cfg.image_pooling_2d == ImagePooling2DType.attention_meanq:
            ...
            query = image_features2.mean(-2, keepdim=True)
            image_features2 = self.image_pooling_2d_2(query, image_features2)
            ...
            if self.grad_checkpointing:
                ...
                image_features2 = checkpoint(self.image_pooling_2d_2, image_features2[:, :1, :], image_features2, use_reentrant=False)
            else:
                image_features = self.image_pooling_2d(image_features[:, :1, :], image_features)
                image_features2 = self.image_pooling_2d_2(image_features2[:, :1, :], image_features2)
        # print(66, image_features.shape)
        h, w = cfg.llm_patches_per_crop()
        image_features = image_features.reshape(batch_size, num_image, h * w, -1)

        h, w = cfg.llm_patches_per_crop2()
        image_features2 = image_features2.reshape(batch_size, num_image, h * w, -1)
        
        ## Apply Cross-rope features Concatination
        
        cross_img_features = self.cross_rope(
        ...
        cross_img_features = cross_img_features['sig_fused']
        ...
        # if cfg.image_projector2 == ImageProjectType.mlpx2:
        #     # print(6)
        #     for module in self.image_projector:
        #         image_features2 = module(image_features2)
        # else:
        #     # print(7)
        #     if self.grad_checkpointing:
        #         from torch.utils.checkpoint import checkpoint
        #         image_features2 = checkpoint(self.image_projector2, image_features2, use_reentrant=False)
        #     else:
        #         image_features2 = self.image_projector2(image_features2)

        # # print(88, image_features.shape)
        # image_features_cat = torch.cat([image_features, image_features2], dim=2)
        # if rank == 0:v 1
        # breakpoint()
        return cross_img_features # image_features_cat   #image_features
    ...
    #     image_features = self.image_feature_dropout(image_features)
    #     print(22, image_features.shape)
    #     image_features = self.merger(image_features, image_features2) 
    #     print(222, image_features.shape)
        
    #     # After merger, we need to calculate the new patch dimensions
    #     merged_num_patches = image_features.shape[2]  # 1258 in your case
    #     merged_feature_dim = image_features.shape[3]  # 1024 in your case
        
    #     # Calculate the square root to get the height/width dimensions
    #     # Since 1258 is not a perfect square, we need to find the closest factorization
    #     # or handle it differently based on how your merger works
        
    #     # Option 1: If merger concatenates patches, calculate new grid size
        
    #     merged_h = int(math.sqrt(merged_num_patches))
    #     merged_w = merged_num_patches // merged_h
        
    #     # Adjust if not exact
    #     while merged_h * merged_w != merged_num_patches:
    #         merged_h += 1
    #         merged_w = merged_num_patches // merged_h
    #         if merged_h * merged_w > merged_num_patches:
    #             # If we can't get exact factorization, we might need padding
    #             break
        
    #     # If exact factorization isn't possible, pad to nearest square
    #     if merged_h * merged_w != merged_num_patches:
    #         target_size = int(math.ceil(math.sqrt(merged_num_patches))) ** 2
    #         padding_needed = target_size - merged_num_patches
    #         # Pad the patch dimension
    #         image_features = F.pad(image_features, (0, 0, 0, padding_needed))
    #         merged_h = merged_w = int(math.sqrt(target_size))
    #         merged_num_patches = target_size
        
    #     # Now reshape with the correct dimensions
    #     image_features = image_features.reshape(batch_size, num_image, merged_h, merged_w, merged_feature_dim)
    #     print(33, image_features.shape)
        
    #     # Continue with pooling using the new dimensions
    #     if merged_h % cfg.image_pooling_h != 0 or merged_w % cfg.image_pooling_w != 0:
    #         pad_h = (cfg.image_pooling_h - merged_h % cfg.image_pooling_h) % cfg.image_pooling_h
    #         pad_w = (cfg.image_pooling_w - merged_w % cfg.image_pooling_w) % cfg.image_pooling_w
    #         # Pad so we can still pool mxn patches
    #         image_features = F.pad(
    #             image_features,
    #             (0, 0, 0, pad_w, 0, pad_h, 0, 0, 0, 0),
    ...
    #     # image pooling with updated dimensions
    #     image_features = einops.rearrange(
    #         image_features,
    #         'b n (h dh) (w dw) c -> (b n h w) (dh dw) c',
    #         dh=cfg.image_pooling_h,
    ...
    #     # Calculate the final h, w after pooling
    #     final_h = merged_h // cfg.image_pooling_h
    #     final_w = merged_w // cfg.image_pooling_w
    #     image_features = image_features.reshape(batch_size, num_image, final_h * final_w, -1)
    #     print(77, image_features.shape)
    ...
    #             # pad_embed2 = self.pad_embed2[:, None, None, None, :]
    #             # all_pad = image_masks == 0
    #             # partial_pad = torch.logical_and(image_masks < 1, torch.logical_not(all_pad)).to(dtype=torch.float32)
    #             # all_pad = all_pad.to(dtype=torch.float32)
    #             # image_features2 = image_features2 + pad_embed2[0] * torch.unsqueeze(all_pad, -1)
    #             # image_features2 = image_features2 + pad_embed2[1] * torch.unsqueeze(partial_pad, -1)
...
class Molmo(nn.Module):
    ...
    @staticmethod
    def get_vit_parameters2():
        return tuple(
            [
                "vision_backbone.image_vit2",
                "beta_logits",
            ]
        )
    ...
    def forward(
        ...
        image_input_idx2: Optional[torch.Tensor] = None,
        ...
        if images is not None:
            # print("AD -> ", image_input_idx.shape)
            # print("AD2 -> ", image_input_idx2.shape)
            # image_input_idx = torch.cat([image_input_idx, image_input_idx2], dim=2)
            # shape: (batch_size, num_image, num_patch, d_model)
            # cls_embed: (batch_size, num_image, d_model)
            # if get_rank() == 0:
            #     pdb.set_trace()
            image_features = self.vision_backbone(images, image_masks)  # first run shape:  images.shape, image_masks.shape= (torch.Size([1, 13, 256, 588]), torch.Size([1, 13, 256])) , output shape: torch.Size([1, 13, 113, 2560])
            num_image, num_patch = image_features.shape[1:3]
            # print(image_input_idx.shape, "-----", batch_size, num_image, num_patch)
    ...
    def generate(
        ...
        def step(
            ...
            if tokens_generated > 0:
                ...
                _image_input_idx2 = image_input_idx2
            ...
            output = self(
                ...
                image_input_idx2=_image_input_idx2,
