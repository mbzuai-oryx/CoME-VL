...
class VisionBackboneConfig(BaseConfig):
    ...
    # def __post_init__(self):
    #     self.image_default_input_size2 = tuple(self.image_default_input_size2)  # type: ignore[assignment]
...
class TokenizerConfig(BaseConfig):
    ...
    # tokenizer_dir: Optional[str] = "/Med_Repo_Gen/molmo/Qwen-tokenizer"
    ...
    def update_legacy_settings(cls, config: D) -> D:
        ...
        for k in ["olmo_eos_token_id", "olmo_bos_token_id", "truncate_direction"]:
            if k in config:
                del config[k]
        # config.identifier = "/Med_Repo_Gen/molmo/Qwen-tokenizer"
        # config.tokenizer_dir = "/Med_Repo_Gen/molmo/Qwen-tokenizer"
        return config
...
class ModelConfig(BaseConfig):
    ...
    # block_type: BlockType = BlockType.llama
    """
    ...
    vision_backbone2: Optional[VisionBackboneConfig] = None
    """
    Vision backbone settings for multi-modal models.
    """

    vit_load_path: Optional[str] = None
    vit_load_path2: Optional[str] = None
    """
    ...
    vit_layers2: Tuple = (-1,)  # TODO should we fix the offset?
    """
    ...
    image_projector2: ImageProjectType = ImageProjectType.mlp
    """
    Projector layer
    """
    ...
    def __post_init__(self):
        ...
        self.vit_layers2 = tuple(self.vit_layers2)  # type: ignore[assignment]
    ...
    def update_legacy_settings(cls, config: D) -> D:
        ...
        if config.vision_backbone2 is not None:
            config.vision_backbone2 = VisionBackboneConfig.update_legacy_settings(config.vision_backbone2)
    ...
    @property
    def image_num_patch2(self):
        if self.vision_backbone2 is None:
            raise ValueError("No vision backbone")
        return self.vision_backbone2.image_num_patch
    ...
    def llm_patches_per_crop(self):
        ...
        return h, w
    def llm_patches_per_crop2(self):
        h, w = self.image_num_patch2
        # Round up in case we need to pad the image features for pooling
        h = (h + self.image_pooling_h - 1) // self.image_pooling_h
        w = (w + self.image_pooling_w - 1) // self.image_pooling_w
        return h, w
...
class EvaluatorConfig(BaseConfig):
    ...
    """If save predictions, also save full decoded output to a separate JSON file"""
