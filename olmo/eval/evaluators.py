...
from .vqa import vqa_score, anls_metric, relaxed_correctness, \
    a_okvqa_score, select_mc_option, mmmu_score, real_world_qa_score, math_vista_score, iou_score
...
class SavePredictions(Evaluator):
    ...
    def __init__(self, output_dir, json=True, save_tokens=True,
                 log_examples=10, save_full_predictions=False):
    ...
    def __call__(self, metadatas, predictions, tokenizer,
        ...
        prompt_tokens = predictions["prompts"]
        json_data = []
        json_data_full = [] if self.save_full_predictions else None
        html_data = []
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            pred_tokens = pred_seq[pred_seq >= 0]
            ...
            if self.save_full_predictions:
                full_text = tokenizer.decode(pred_tokens, truncate_at_eos=False)
                json_row_full = dict(json_row)
                json_row_full["prediction_full"] = full_text
                json_data_full.append(json_row_full)
        ...
        if self.json:
            ...
            if get_world_size() > 1 and self.json:
                if get_global_rank() == 0:
                    ...
                    if self.save_full_predictions:
                        all_predictions_full = [None]*get_world_size()
                        dist.gather_object(json_data_full, all_predictions_full)
                        json_data_full = flatten_list(all_predictions_full)
                else:
                    dist.gather_object(json_data, None)
                    if self.save_full_predictions:
                        dist.gather_object(json_data_full, None)
            ...
            if get_global_rank() == 0:
                ...
                if self.save_full_predictions:
                    json_full_file = os.path.join(
                        self.output_dir,
                        self.get_file_name(step, None) + "_full.json"
                    )
                    with open(json_full_file, "w") as f:
        ...
        for ex_ix, pred_seq in enumerate(new_tokens):
            ...
            for metric in self.metric:
                if metric == "vqa_score":
                    ...
                    # print('responce -> ', answers)
                    # print('prediction -> ', pred)
                    # print('metadata -> ', metadata)
                    # if rank==0:
                    #     breakpoint()
        
                    score = iou_score(answers, pred, metadata)
                else:
