...
_SUB_FOLDER_OR_FILE_NAME = {
    "questions": {
        ...
        "test": "v2_OpenEnded_mscoco_test2014_questions.json",
    },
    "annotations": {
...
class VQAv2BuilderMultiQA(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        # if not os.path.exists(f"{self.vqa_source}/test2014.zip"):
        #     download_file(_URLS_IMAGES["test"], f"{self.vqa_source}/test2014.zip")
            
        if not os.path.exists(f"{self.vqa_source}/train2014"):
            download_file(_URLS_IMAGES["train"], f"{self.vqa_source}/train2014")
        if not os.path.exists(f"{self.vqa_source}/val2014"):
            download_file(_URLS_IMAGES["val"], f"{self.vqa_source}/val2014")
        if not os.path.exists(f"{self.vqa_source}/test2014"):
            download_file(_URLS_IMAGES["test"], f"{self.vqa_source}/test2014")
            
        #
        # downloaded_pointer["images"] = {
        #     "train": f"{self.vqa_source}/train2014.zip",
        #     "val": f"{self.vqa_source}/val2014.zip",
        #     "test": f"{self.vqa_source}/test2014.zip",
        # }
        downloaded_pointer["images"] = {
            "train": os.path.join(self.vqa_source, "train2014"),
            "val": os.path.join(self.vqa_source, "val2014"),
            "test": os.path.join(self.vqa_source, "test2014"),
        }
