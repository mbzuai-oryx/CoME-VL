...
class Optimizer(OptimizerBase):
    ...
    def clip_grads_and_collect_metrics(
        ...
        per_param_group_names: List[str] = []
        ...
        for group in self.param_groups:
            ...
            for name, p in zip(group["param_names"], group["params"]):
                ...
                for x, prefix in zip(tensors, prefixes):
                    ...
                    per_param_group_names.append(group_name)
        ...
        if multi_modal:
            assert len(per_param_norm_metrics) == len(per_param_group_names)

        def is_grad_norm_metric(metric_name: str) -> bool:
            return metric_name.startswith("grad/") and metric_name.endswith(".norm")

        def is_connector(group_name: str) -> bool:
            return "connector" in group_name

        def is_vit(group_name: str) -> bool:
            return "vit" in group_name
    ...
    def _do_global_fixed_clipping(
        ...
        if multi_modal:
            ...
            if "connector" in group_name:
                total_grad_norm = all_metrics["connector_grad_norm"]
            elif "vit" in group_name:
...
def get_multimodal_param_groups(cfg: TrainConfig, model: nn.Module) -> List[Dict[str, Any]]:
    ...
    vit_params2 = list(Molmo.get_vit_parameters2())
    vit_params.extend(vit_params2)
    ...
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            # is_vit2 = listinstr(vit_params2, fpn, delimiter='.')
            ...
            elif is_connector and no_apply_decay:
                connector_no_decay.add(fpn)
            elif is_vit and not no_apply_decay:
                vit_decay.add(fpn)
            elif is_vit and no_apply_decay:
                vit_no_decay.add(fpn)
            # elif is_vit2 and not no_apply_decay:
            #     vit_decay2.add(fpn)
            # elif is_vit2 and no_apply_decay:
            #     vit_no_decay2.add(fpn)
            elif is_llm and not no_apply_decay:
                llm_decay.add(fpn)
    ...
    connector_no_decay_sorted = sorted(list(connector_no_decay))
    vit_decay_sorted = sorted(list(vit_decay))
    vit_no_decay_sorted = sorted(list(vit_no_decay))
    llm_decay_sorted = sorted(list(llm_decay))
    llm_no_decay_sorted = sorted(list(llm_no_decay))
    param_groups = []
    if len(connector_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in connector_decay_sorted],
                "param_names": connector_decay_sorted,
                "lr": cfg.optimizer.connector_learning_rate,
                "weight_decay": cfg.optimizer.connector_weight_decay,
                "betas": cfg.optimizer.connector_betas,
                "eps": cfg.optimizer.connector_eps,
                "group_name": "connector_decay",
                **param_group_defaults,
            }
        )
    if len(connector_no_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in connector_no_decay_sorted],
                "param_names": connector_no_decay_sorted,
                "lr": cfg.optimizer.connector_learning_rate,
                "weight_decay": 0.0,
                "betas": cfg.optimizer.connector_betas,
                "eps": cfg.optimizer.connector_eps,
                "group_name": "connector_no_decay",
                **param_group_defaults,
            }
        )
    if len(vit_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in vit_decay_sorted],
                "param_names": vit_decay_sorted,
                "lr": cfg.optimizer.vit_learning_rate,
                "weight_decay": cfg.optimizer.vit_weight_decay,
                "betas": cfg.optimizer.vit_betas,
                "eps": cfg.optimizer.vit_eps,
                "group_name": "vit_decay",
                **param_group_defaults,
            }
        )
    if len(vit_no_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in vit_no_decay_sorted],
                "param_names": vit_no_decay_sorted,
                "lr": cfg.optimizer.vit_learning_rate,
                "weight_decay": 0.0,
                "betas": cfg.optimizer.vit_betas,
                "eps": cfg.optimizer.vit_eps,
    ...
    if len(llm_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in llm_decay_sorted],
                "param_names": llm_decay_sorted,
                "lr": cfg.optimizer.llm_learning_rate,
                "weight_decay": cfg.optimizer.llm_weight_decay,
                "betas": cfg.optimizer.llm_betas,
                "eps": cfg.optimizer.llm_eps,
                "group_name": "llm_decay",
                **param_group_defaults,
            }
        )
    if len(llm_no_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in llm_no_decay_sorted],
                "param_names": llm_no_decay_sorted,
                "lr": cfg.optimizer.llm_learning_rate,
                "weight_decay": 0.0,
                "betas": cfg.optimizer.llm_betas,
                "eps": cfg.optimizer.llm_eps,
...
def build_optimizer(cfg: TrainConfig, model: nn.Module) -> Optimizer:
    if cfg.model.vision_backbone is not None:
        param_groups = get_multimodal_param_groups(cfg, model)
    else:
...
def build_multimodal_scheduler(cfg: TrainConfig):
    ...
    connector_sched = CosWithWarmup(
        ...
        warmup_steps=int(sched_cfg.connector_t_warmup),
    ...
    vit_sched = CosWithWarmup(
        ...
        grad_clip_warmup_factor=sched_cfg.grad_clip_warmup_factor,
        warmup_steps=int(sched_cfg.vit_t_warmup),
        alpha_f=sched_cfg.alpha_f,
        t_max=None if sched_cfg.t_max is None else int(sched_cfg.t_max),
        warmup_min_lr=sched_cfg.warmup_min_lr,
    )
    llm_sched = CosWithWarmup(
        ...
        grad_clip_warmup_factor=sched_cfg.grad_clip_warmup_factor,
        warmup_steps=int(sched_cfg.llm_t_warmup),
        alpha_f=sched_cfg.alpha_f,
        t_max=None if sched_cfg.t_max is None else int(sched_cfg.t_max),
        warmup_min_lr=sched_cfg.warmup_min_lr,
    )
