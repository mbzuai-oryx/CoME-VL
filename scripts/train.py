...
if dist.is_initialized():
    dist.barrier()  # align all ranks before break
...
def main(cfg: TrainConfig) -> None:
    ...
    if cfg.model.vision_backbone is not None and not cfg.ft_vit2:
        log.info(f"Freezing vision backbone2")
        freeze_parameters_by_name(olmo_model, Molmo.get_vit_parameters2(), warn=False)
    if not cfg.ft_llm:
    ...
    if cfg.load_path is None:
        ...
        if get_local_rank() == 0:
            if cfg.model.init_device == "meta":
                ...
                keep_vit2 = olmo_model.vision_backbone.image_vit2          # save ref
                olmo_model.vision_backbone.image_vit2 = nn.Identity()      # placeholder so it's skipped

                # Empty the rest of the model
                olmo_model.to_empty(device="cpu")

                # put it back
                olmo_model.vision_backbone.image_vit2 = keep_vit2
            # state_dict = torch.load(join("/vast/users/annonymus/Document/molmo_code/Molmo-7B-D-0924/model.pt"), map_location="cpu")
            ...
            if cfg.initial_model_checkpoint:
                ...
                # olmo_model.load_state_dict(state_dict, strict=True)
                
                # Load checkpoint
                state_dict = torch.load(join(cfg.initial_model_checkpoint, "model.pt"), map_location="cpu")

                # Remove vision_backbone.alpha_logits and beta_logits to skip loading them
                # if 'vision_backbone.alpha_logits' in state_dict:
                #     del state_dict['vision_backbone.alpha_logits']
                #     print("Skipped loading vision_backbone.alpha_logits - will use model's initialized value")

                # if 'vision_backbone.beta_logits' in state_dict:
                #     del state_dict['vision_backbone.beta_logits']
                #     print("Skipped loading vision_backbone.beta_logits - will use model's initialized value")

                # Load the modified state dict
                missing_keys, unexpected_keys = olmo_model.load_state_dict(state_dict, strict=True)
    ...
    fsdp_model = FSDP(
        ...
        # forward_prefetch=True,   # Added to avoid flatten shape auto reset
